...
Array_HOME: Array[1..1] of Campos =
(  {JOGO} {CONFIG}                {EXE}
  ('HOME','HOME\home_dosbox.conf','PLAY.exe' )
);
...
AbfWav1.FileName:=DIR_App+'BIN\home_voice.wav';
...
begin
  Try
  CopyFile(PChar(Form1.DIR_App+'BIN\dosbox-0.74.conf'),PChar(Array_HOME[1][2]),True);
  Finally
  AbfWav1.Play;
  end;
end;
...
Arquivo_DOSBOX_Fisico.LoadFromFile(DIR_APP+Array_HOME[1][2]);
   ...
   begin
    ...
    Arquivo_DOSBOX_Fisico.Add('mount c "'+DIR_App+Array_HOME[1][1]+'"');
    Arquivo_DOSBOX_Fisico.Add('c:');
    Arquivo_DOSBOX_Fisico.Add('cls');
    Arquivo_DOSBOX_Fisico.Add(Array_HOME[1][3]);
    Arquivo_DOSBOX_Fisico.Add('Exit.');
    end;

   end;

Arquivo_DOSBOX_Fisico.SaveToFile(DIR_APP+Array_HOME[1][2]);
Arquivo_DOSBOX_Fisico.Free;

ShellExecute(Handle,'open',pchar(DIR_App+'BIN\DOSBox.exe')
                          ,pchar('-conf '+Array_HOME[1][2]+' -noconsole -c "exit"')
...
begin
DeleteFile(DIR_App+Array_HOME[1][2]);
