...
Array_SIERRA: Array[1..1] of Campos =
(  {JOGO}                  {RESOURCE}       {CONFIG}             {EXE}
  ('SIERRA\Phantasmagoria','.\resource.cfg','sierra_dosbox.conf','SIERRA.exe')
);
...
AbfWav1.FileName:=DIR_App+'BIN\sierra_voice.wav';
...
begin
  Try
  CopyFile(PChar(Form1.DIR_App+'BIN\dosbox-0.74.conf'),PChar(Array_SIERRA[1][3]),True);
  Finally
  AbfWav1.Play;
  end;
end;
...
Arquivo_DOSBOX_Fisico.LoadFromFile(DIR_APP+Array_SIERRA[1][3]);
   ...
   begin
    ...
    Arquivo_DOSBOX_Fisico[i]:='memsize=8';
    ...
    if Pos('sbtype=',Arquivo_DOSBOX_Fisico[i]) = 1 then
    Arquivo_DOSBOX_Fisico[i]:='sbtype=sb1';
    ...
    Arquivo_DOSBOX_Fisico.Add('mount c "'+DIR_App+Array_SIERRA[1][1]+'"');
    Arquivo_DOSBOX_Fisico.Add('cls');
    Arquivo_DOSBOX_Fisico.Add('c:');
    Arquivo_DOSBOX_Fisico.Add('sierra '+Array_SIERRA[1][2]);
    Arquivo_DOSBOX_Fisico.Add('Exit.');
    end;

   end;

Arquivo_DOSBOX_Fisico.SaveToFile(DIR_APP+Array_SIERRA[1][3]);
Arquivo_DOSBOX_Fisico.Free;

ShellExecute(Handle,'open',pchar(DIR_App+'BIN\DOSBox.exe')
                          ,pchar('-conf '+Array_SIERRA[1][3]+' -noconsole -c "exit"')
...
begin
DeleteFile(DIR_App+Array_SIERRA[1][3]);
